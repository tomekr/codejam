...
word_length, word_lines, cases = input.shift.split(' ').map(&:to_i)

words = word_lines.times.map{ input.shift }

(1..cases).each do |case_number|
  radio_message = input.shift.gsub('(', '[').gsub(')',']')
  reg = Regexp.new radio_message

  puts "Case ##{ case_number }: #{ words.grep(reg).size }"
end
...
In order to help them decipher these messages, the scientists have asked you
to devise an algorithm that will determine the number of possible
